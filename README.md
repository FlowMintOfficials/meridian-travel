...
- **Backup & restore** the entire dataset as a single JSON file; import merges by id rather than clobbering. Destructive actions (import, wipe, delete) go through in-app confirmation dialogs — no native browser popups. An **encrypted backup** option (Settings → Export encrypted) wraps the whole export in AES-GCM under a one-time passphrase — separate from the docs vault passphrase, useful when the file itself might pass through email or a shared drive.
...
The encrypted-documents vault (passport scans, boarding passes, notes) uses AES-GCM with PBKDF2-derived keys — the passphrase never leaves your device, and only a verifier (not the key itself) is stored. The same primitives back the optional encrypted full-backup export (Settings → Export encrypted): a one-time passphrase you set at export time, never stored, wraps the entire backup JSON in AES-GCM. Forget either passphrase and there's no recovery — that's the tradeoff for nothing being recoverable by anyone else either.
...
- [x] Optional AES-GCM-encrypted full-backup export/import, independent of the docs vault passphrase
